var myApp = angular.module('calendarDemoApp', ['ngMaterial','ui.calendar', 'ui.bootstrap','toaster', 'ngAnimate',"page.loading","page.services","page.summary"]);
...
function CalendarCtrl($scope, $rootScope, $compile, $timeout, uiCalendarConfig, toaster, $http, $timeout) {
    var date = new Date();
    var d = date.getDate();
    var m = date.getMonth();
    ...
    $rootScope.actualServiceDuration=null;
    $rootScope.actualServiceName=null;
    ...
    $timeout(function(){$rootScope.activePage= 'loading'; $scope.showCalendar=true;}, 0);
    $timeout(function(){$rootScope.activePage= 'services'}, 10);
    ...
    //$sce.trustAsResourceUrl("http://localhost:8080/prenotami2/events.json");
    //
    ...
    $scope.createEvents = function(){
    	$http.get( 'http://spagobi-poc.eng.it/SpagoBI/publicjsp/events.json').then(function successCallback(response) {
    			uiCalendarConfig.calendars['myCalendar1'].fullCalendar('removeEvents');
    			uiCalendarConfig.calendars['myCalendar1'].fullCalendar('addEventSource',response.data);
  ...
  //TO-DO devono non essere editabili: editable=false
    ...
    $scope.checkDay = function(date) {
    	if( $scope.isClosed(date)){
			toaster.error("", "Il negozio è chiuso nel giorno selezionato. Scegliere un altro giorno.");
    ...
    $scope.viewRender = function(view, element){
    	if(view.name=="agendaDay"){
    		element.addClass($scope.getDayAdditionalClass(view.start));
    	}
    }
